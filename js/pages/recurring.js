...
window.Ledger.pages.renderRecurringPage = function(){
  ...
  var rows = window.Ledger.DB.recurring.slice().sort(function(a,b){ return window.Ledger.nextDueDate(a,today).localeCompare(window.Ledger.nextDueDate(b,today)); }).map(function(r){
    ...
    return '<div class="bill-row">'
      ...
      + '  <button class="icon-btn danger" data-del-recurring="' + r.id + '" title="Remove" aria-label="Remove">' + window.Ledger.iconTrash() + '</button>'
      + '</div>'
      + '</div>';
  }).join("");
  ...
  return ''
    + '<div class="card">'
    + '  ' + window.Ledger.cardHeader('Recurring transactions', '<span class="hint">confirm-to-post or auto-post when due</span>')
